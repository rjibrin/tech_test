...
- Database schema has 3 tables (found in `server/prisma/schema.prisma`): 
  ...
  - Table `Game` contains non-player specific information on past games. Games that were prematurely ended without a conclusion are not stored.
...
- Leaderboard is computed in two ways:
  ...
  - `/api/players/leaderboard/queryraw` uses SQL to efficiently aggregate data in the database. Speedup apparent for large databases such as `prisma/largeSeed.ts`.
...
- Added disable logic to prevent unwanted moves or changes, e.g., no moves after game ends, cannot select taken cell, prohibit changing settings during game
...
- Added tests for game logic in `client/src/__tests__/checkBoard.test.ts`
- Select elements for players and board size
- Leaderboard table in a separate "page" from game board
